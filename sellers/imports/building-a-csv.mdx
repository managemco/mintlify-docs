...
<Tip>
  **New to URNs?** A URN is just Managem's permanent code for a specific card or
  sealed product — for example `urn:managem:pokemon:set/surging-sparks/card/1`.
  You'll find the URN on the item's page on [managem.co.uk](https://managem.co.uk)
  (it's shown on the detail panel and is included in the item's URL). Using a
  URN in your CSV is the fastest and most reliable way to match a row to the
  right catalogue item. If you'd rather not deal with URNs, you can identify
  rows with `GAME + SET + NAME` or an external marketplace ID instead — see the
  examples below. The full URN reference lives at [URNs](/marketplace/data/urn).
</Tip>
